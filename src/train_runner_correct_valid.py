...
def train(
    ...
    for epoch_num in range(resume_epoch + 1, epochs):
        check_point_file = f"{checkpoints_dir}/{model_name}_{epoch_num:03}.pt"
        print("load check_point_file from: ", check_point_file)
        retinanet = torch.load(check_point_file).cuda()
        retinanet = torch.nn.DataParallel(retinanet).cuda()
#         retinanet.train()
#         if epoch_num < 1:
...
def validation(
    ...
    with torch.no_grad():
        ...
        if save_oof:  # save predictions
            pickle.dump(oof, open(f"{predictions_dir}/valid_all_{epoch_num:03}.pkl", "wb"))
...
def check_metric(
    ...
    for epoch_num in range(start_epoch, end_epoch):
        fn = f"{predictions_dir}/valid_all_{epoch_num:03}.pkl"
        try:
        ...
        for threshold in thresholds:
            ...
            for img_id in range(nb_images):
                for batch_id in range(batch_size):
                    ...
                    gt_boxes = oof["gt_boxes"][img_id][batch_id].copy()
                    # print("oof[boxes][img_id]: {}".format(oof["boxes"][img_id]))
                    boxes = oof["boxes"][img_id][batch_id].copy()
                    scores = oof["scores"][img_id][batch_id].copy()
