...
def nms_whl(dets, thresh, variance=None, soft_nms=False):
...
class RetinaNet(nn.Module):
    ...
    def boxes(self, img_batch, regression, classification, global_classification, anchors):
        ...
        batch_size = transformed_anchors.shape[0]
        nms_bboxs, nms_scores = [], []
        for batch in range(batch_size):

            scores_over_thresh = (scores > 0.025)[batch, :, 0]
            if scores_over_thresh.sum() == 0:
                # no boxes to NMS, just return
                nms_scores.append(torch.zeros(0).cpu().numpy().copy())
                nms_bboxs.append(torch.zeros(0, 4).cpu().numpy().copy())
            else:
                transformed_anchor = transformed_anchors[batch, scores_over_thresh, :]
                score = scores[batch, scores_over_thresh, :]

                # use very low threshold of 0.05 as boxes should not overlap
                nms_bbox, nms_score, _ = nms_whl(torch.cat([transformed_anchor, score], dim=1),
                                                 0.05,
                                                 soft_nms=False
                                                 )
                nms_bboxs.append(nms_bbox.cpu().numpy().copy())
                nms_scores.append(nms_score.cpu().numpy().copy())
                # nms_bbox [Batch, anchor_num, 4], nms_scores [Batch, anchor_num, 1]
        return [nms_scores, global_classification, nms_bboxs]
    ...
    def forward(self, inputs, return_loss, return_boxes, return_raw=False):
        ...
        if return_raw:
            return [regression, classification, torch.tensor(0.).cuda(), anchors]
        ...
        if return_boxes:
            res += self.boxes(img_batch=img_batch,
                              ...
                              global_classification=torch.tensor(0.).cuda(),
                              anchors=anchors)
