...
def train(
    ...
    for epoch_num in range(resume_epoch + 1, epochs):
        ...
        # set losses
        epoch_loss, loss_cls_hist, loss_cls_global_hist, loss_reg_hist, loss_distribution_hist = [], [], [], [], []
        ...
        with torch.set_grad_enabled(True):
            ...
            for iter_num, data in data_iter:
                ...
                # get losses
                (classification_loss, regression_loss, distribution_loss, global_classification_loss,) = retinanet(
                    inputs, return_loss=True, return_boxes=False
                )
                classification_loss = classification_loss.mean()
                regression_loss = regression_loss.mean()
                global_classification_loss = global_classification_loss.mean()
                loss = classification_loss + regression_loss + global_classification_loss * 0.1 + distribution_loss / 4
...
#                 loss = classification_loss * 0.1 + regression_loss
                ...
                loss_distribution_hist.append(float(distribution_loss))
                epoch_loss.append(float(loss))
                # print losses with tqdm interator
                data_iter.set_description(
                    f"{epoch_num} cls: {np.mean(loss_cls_hist):.4f} cls g: {np.mean(loss_cls_global_hist):.4f} Reg: {np.mean(loss_reg_hist):.4f} Dist: {np.mean(loss_distribution_hist):.4f} Loss: {np.mean(epoch_loss):.4f}"
                )
...
def validation(
    ...
    with torch.no_grad():
        ...
        for iter_num, data in data_iter:
            ...
            loss_distribution_hist_valid.append(float(distribution_loss))
            data_iter.set_description(
                f"{epoch_num} cls: {np.mean(loss_cls_hist_valid):1.4f} cls g: {np.mean(loss_cls_global_hist_valid):1.4f} Reg: {np.mean(loss_reg_hist_valid):1.4f} Dist: {np.mean(loss_distribution_hist_valid):.4f} Loss {np.mean(loss_hist_valid):.4f}"
            )
            del classification_loss
            del regression_loss
...
def check_metric(
    ...
    # plt.savefig(f"{RESULTS_DIR}/scores/{model_name}{run_str}_fold_{fold}/score_fig.png")
    if save_metrics:
...
def main():
    ...
    arg("--resume_weights", type=str, default='', help="directory with weights (if avaialable)")
    arg("--resume_epoch", type=int, default=0, help="number of epoch to continue training")
